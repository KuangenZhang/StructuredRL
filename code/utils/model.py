...
def regression(loader, net, device, loss_func = None, optimizer = None, is_train = True):
    mean_loss = 0.0
    for x, y in loader:
...
def regression_FA(loader, feature_net, mapper_net, device,
                  loss_func = None, feature_optimizer = None, mapper_optimizer = None,
                  is_train = True):
    mean_loss = 0.0
    for x, y in loader:
        x, y = x.float().to(device), y.float().to(device)
        batch_size = x.size()[0]
        prediction = mapper_net(feature_net(x))  # input x and predict based on x
        loss = loss_func(prediction, y)  # must be (1. nn output, 2. target)
        if is_train:
            feature_optimizer.zero_grad()  # clear gradients for next train
            mapper_optimizer.zero_grad()  # clear gradients for next train
            loss.backward()  # backpropagation, compute gradients
            mapper_optimizer.step()  # apply gradients
            feature_optimizer.step()  # apply gradients
        mean_loss += loss * batch_size
    return mean_loss / len(loader.dataset)
